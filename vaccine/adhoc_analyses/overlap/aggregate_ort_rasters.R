...
ort<- lapply(cov_year_list, function(x){
  f<-raster(paste0('<FILEPATH>/IHME_LMIC_ORT_2000_2017_ORT_PERCENT_MEAN/IHME_LMIC_ORT_2000_2017_ORT_PERCENT_MEAN_',x,'_Y2020M05D27.TIF'))
})
ort<-brick(ort)
...
if(length(year_list)!=length(cov_year_list)){
  ...
  for(x in 1:m){
    ort<-addLayer(ort,ort[[1]])
  }
}

ortc<- lapply(cov_year_list, function(x){
  f<-raster(paste0('<FILEPATH>/IHME_LMIC_ORT_2000_2017_ORT_NUMBER_MEAN/IHME_LMIC_ORT_2000_2017_ORT_NUMBER_MEAN_',x,'_Y2020M05D27.TIF'))
})
ortc<-brick(ortc)
...
if(length(year_list)!=length(cov_year_list)){
  m<-length(year_list) -length(cov_year_list)
  for(x in 1:m){
    ortc<-addLayer(ortc,ortc[[1]])
  }
}


#Subset orting down to the region
ort<-crop(ort,simple_raster)
ort<-extend(ort, extent(simple_raster))
ort<-raster::mask(ort,simple_raster)


ortc<-crop(ortc,simple_raster)
ortc<-extend(ortc, extent(simple_raster))
ortc<-raster::mask(ortc,simple_raster)
...
#Merge in ort
# #merge ort percent and counts first to deal w some NAs
ortv<-as.vector(ort)
sv<-as.vector(simple_raster)
ortv<-ortv[!is.na(sv)]
ortcv<-as.vector(ortc)
ortcv<-ortcv[!is.na(sv)]
#ortcv[is.na(ortv)]<-0
# ortv[is.na(ortv)] <- 0
...
cell_pred_ort <- data.table(rep(1,(length(year_list) * length(unique(link$ID)))))
cell_pred_ort[, cell_pred_id := .I] #cell_pred_ort object ID
cell_pred_ort[,cell_id := rep(cell_ids, times = nrow(cell_pred_ort) / length(cell_ids))]  #cell id references the africa map
cell_pred_ort[,pixel_id := rep(pixel_id, times = nrow(cell_pred_ort) / length(pixel_id))] #pixel id references the regional map
cell_pred_ort[,ort := ortv] #add ort percent data
cell_pred_ort[,ortc := ortcv] #add ort count data


cell_pred = cbind(cell_pred, cell_pred_ort[,list(ort,ortc)])

overs <- paste0("V", 1:500)

#Align missing pixels from the cell pred and new covariate
cell_pred[is.na(ort) & !is.na(V2),(overs):=NA]
cell_pred[!is.na(ort) & is.na(V2),ort:=NA]
cell_pred[!is.na(ortc) & is.na(V2),ortc:=NA]
cell_pred[is.na(ort),c('pop','cov_pop','dtp0_pop'):=NA]
...
cell_pred <- cell_pred[, ortc := ortc * area_fraction]
...
cell_pred <- cell_pred[, ort := lapply('ort', function(x) get(x) * cov_pop) ]
############################################################
# creating a raked counts aggregations
message("creating a raked counts aggregations")

overs <- c(overs,'dtp3_pop','ort','cov_pop','dtp0_pop','ortc')
...
admin_0_dtp3 <- admin_0_dtp3[, ort_pct := lapply(.SD, function(x) x / cov_pop), .SDcols='ort']
admin_1_dtp3 <- admin_1_dtp3[, ort_pct := lapply(.SD, function(x) x / cov_pop), .SDcols='ort']
admin_2_dtp3 <- admin_2_dtp3[, ort_pct := lapply(.SD, function(x) x / cov_pop), .SDcols='ort']

admin_0_dtp3<-admin_0_dtp3[,list(dtp3_pop,cov_pop,dtp0_pop,dtp3=mean,ort_pct,ort=ortc,
                                 year,ADM0_CODE)]
admin_1_dtp3<-admin_1_dtp3[,list(dtp3_pop,cov_pop,dtp0_pop,dtp3=mean,ort_pct,ort=ortc,
                                 year,ADM0_CODE,ADM1_CODE)]
admin_2_dtp3<-admin_2_dtp3[,list(dtp3_pop,cov_pop,dtp0_pop,dtp3=mean,ort_pct,ort=ortc,
...
## save aggregations
save_dir <- paste0('<FILEPATH>/zerodose_overlap_analyses/',run_date,'/ort/')
