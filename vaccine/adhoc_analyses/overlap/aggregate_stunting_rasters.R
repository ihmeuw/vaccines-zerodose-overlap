...
stunting<- lapply(cov_year_list, function(x){
  f<-raster(paste0('<FILEPATH>/IHME_GLOBAL_CGF_2000_2019_STUNTING_PREV_PERCENT_A1_S3_MEAN_',x,'_Y2020M08D31.TIF'))
})
stunting<-brick(stunting)
stunting<-stunting/100

if(length(year_list)!=length(cov_year_list)){
  m<-length(year_list) -length(cov_year_list)
  for(x in 1:m){
    stunting<-addLayer(stunting,stunting[[1]])
  }
}

#Subset stuntinging down to the region
stunting<-crop(stunting,simple_raster)
stunting<-extend(stunting, extent(simple_raster))
stunting<-raster::mask(stunting,simple_raster)
...
#Merge in stunting
# #merge stunting percent and counts first to deal w some NAs
stuntingv<-as.vector(stunting)
sv<-as.vector(simple_raster)
stuntingv<-stuntingv[!is.na(sv)]
# stuntingv[is.na(stuntingv)] <- 0

remove(stunting)
gc()

cell_pred_stunting <- data.table(rep(1,(length(year_list) * length(unique(link$ID)))))
cell_pred_stunting[, cell_pred_id := .I] #cell_pred_stunting object ID
cell_pred_stunting[,cell_id := rep(cell_ids, times = nrow(cell_pred_stunting) / length(cell_ids))]  #cell id references the africa map
cell_pred_stunting[,pixel_id := rep(pixel_id, times = nrow(cell_pred_stunting) / length(pixel_id))] #pixel id references the regional map
cell_pred_stunting[,stunting := stuntingv] #add stunting percent data


cell_pred = cbind(cell_pred, cell_pred_stunting[,list(stunting)])

overs <- paste0("V", 1:500)

#Align missing pixels from the cell pred and new covariate
cell_pred[is.na(stunting) & !is.na(V2),(overs):=NA]
cell_pred[!is.na(stunting) & is.na(V2),stunting:=NA]
cell_pred[is.na(stunting),c('pop','cov_pop','dtp0_pop'):=NA]
...
remove(cell_pred_stunting, stuntingv, sv)
gc()
#################################################################################################
...
cell_pred <- cell_pred[, stunting := lapply('stunting', function(x) get(x) * cov_pop) ]
############################################################
# creating a raked counts aggregations
message("creating a raked counts aggregations")

overs <- c(overs,'dtp3_pop','stunting','cov_pop','dtp0_pop')
...
admin_0_dtp3[,dtp3:=apply(.SD,1,  mean, na.rm=T), .SDcols=overs]
admin_1_dtp3[,dtp3:=apply(.SD,1,  mean, na.rm=T), .SDcols=overs]
admin_2_dtp3[,dtp3:=apply(.SD,1,  mean, na.rm=T), .SDcols=overs]


admin_0_dtp3 <- admin_0_dtp3[, stunting_pct := lapply(.SD, function(x) x / cov_pop), .SDcols='stunting']
admin_1_dtp3 <- admin_1_dtp3[, stunting_pct := lapply(.SD, function(x) x / cov_pop), .SDcols='stunting']
admin_2_dtp3 <- admin_2_dtp3[, stunting_pct := lapply(.SD, function(x) x / cov_pop), .SDcols='stunting']

admin_0_dtp3<-admin_0_dtp3[,list(dtp3_pop,cov_pop,dtp0_pop,dtp3,stunting_pct,stunting,
                                 year,ADM0_CODE)]
admin_1_dtp3<-admin_1_dtp3[,list(dtp3_pop,cov_pop,dtp0_pop,dtp3,stunting_pct,stunting,
                                 year,ADM0_CODE,ADM1_CODE)]
admin_2_dtp3<-admin_2_dtp3[,list(dtp3_pop,cov_pop,dtp0_pop,dtp3,stunting_pct,stunting,
...
admin_0_dtp12[,dtp12:=apply(.SD,1,  mean, na.rm=T), .SDcols=overs]
admin_1_dtp12[,dtp12:=apply(.SD,1,  mean, na.rm=T), .SDcols=overs]
admin_2_dtp12[,dtp12:=apply(.SD,1,  mean, na.rm=T), .SDcols=overs]

admin_0_dtp12<-admin_0_dtp12[,list(dtp12,year,ADM0_CODE)]
admin_1_dtp12<-admin_1_dtp12[,list(dtp12,year,ADM0_CODE,ADM1_CODE)]
admin_2_dtp12<-admin_2_dtp12[,list(dtp12,year,ADM0_CODE,ADM2_CODE)]
...
## save aggregations
save_dir <- paste0('<FILEPATH>/zerodose_overlap_analyses/',run_date,'/stunting/')
