load_and_crop_covariates_annual <- function(covs,     
  ...
  if(interval_mo %in% c(12,24,60)){
    ...
    stop('THIS FUNCTION IS NOT CURRENTLY SET UP TO DO MONTHLY')
  }
  ...
  ## CHECK: look through fixed effects and make sure we have covariates available
  ...
  for(c in covs){
    ...
    if(!dir.exists(paste0(covdir,c,'/',measures[i],'/',cov_date,'/',dur,'/'))){
      if(dir.exists(paste0(covdir,c,'/',measures[i],'/',cov_date,'/','synoptic'))){
    ...
    if( durtmp != 'synoptic' ){ ## if we have different rasters for different times
      ...
      all.file.pers <- list.files(paste0(covdir,c,'/',measures[i],'/',cov_date,'/',durtmp,'/'))
    ...
    for(p in 1:perstmp){
      ...
      f <- paste0(covdir,c,'/',measures[i],'/',cov_date,'/',durtmp,'/',c,'_',measures[i],'_',yrtmp,'.tif')
