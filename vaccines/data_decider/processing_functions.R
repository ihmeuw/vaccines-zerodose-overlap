...
prep_for_tabulation <- function(nid, team="gbd", vaccines.=vaccines, filter_table=NA) {
  ...
  ### changed to fix error with the get() function and rotac. Because rotac doesn't contain a numeric dose value, gsub
  ### returns NULL, get() throws error (doesn't return Boolean so == 1 and == 0 don't work)
  if (team=="gbd") {
    for (ea in c(vaccines, "rotac")) {
      #if (ea %in% names(data)) data[get(paste0(gsub("[0-9]", "", ea), "_card"))==1, (paste0(ea, "_CARD")) := get(ea)]
      #if (ea %in% names(data)) data[get(paste0(gsub("[0-9]", "", ea), "_card"))==0 & !is.na(get(ea)), (paste0(ea, "_RECALL")) := get(ea)]
...
## Record data dropped at each stage of filtering (ie includes data data, age is within range,
## includes geospatial data, etc.) to be used in publications data flow flowcharts. When
## loading, creates new empty row to make filling easier
...
#               GBD - Collapse by GBD location ids, save in 'FILEPATH'
# Run:     source("FILEPATH")
...
load.settings <- function(path, topic=NA) {
  ...
  settings <- lapply(names(df), function(n) {
    ...
    if (n=="custom_age_cuts") {
      ...
      if (grepl("<FILEPATH>", i)) i <- gsub("<FILEPATH>", j, i)
...
setup.design <- function(df, var) {
  ...
  ### conservative adjustment for single-PSU strata.  Centers the data for the single-PSU stratum 
...
collapse.subset <- function(df, var, by_vars, design_vars) {
  ...
  # drop rows in groupings with only 1 row (lonely)
...
process_mics <- function(nid) {
  ...
  for (vacc in vaccines_all_in_dataset) {
    ...
    if(unique(data$survey_name) %in% c("UNICEF_MICS")){
      ...
      if (paste0(card_vacc_root, "_day") %in% names(data)) {
        ...
        if (dose_num == 1) {
          # data[get(paste0(card_vacc_root, "_day")) == 0 & get(paste0(vacc_stem, "_card")) == 1, paste0(vacc_stem, "_dose_from_card") := 0]
  ...
  if(unique(data$survey_name) %in% c("UNICEF_MICS")){
    for (vacc_stem in all_stems) {
      ...
      if (paste0(vacc_stem, "_dose") %in% names(data)) {
        ...
        # missing has not been assigned a dose for card, not being assigned a dose for recall either,
        # they are also missing a card and its variables and the presence of the recall_ever AND recall_times
  ...
  ### create indicator for DTP timeliness
  ...
  ### consider inserting check to compare with beginning of file
  ...
  # Drop Point 1: Drop data fpr from age missingness and age range
...
prep_for_tabulation_mics_special_age <- function(nid, team="gbd", vaccines.=vaccines, filter_table=NA, age_start, age_end) {
  ...
  if (check) {
    ...
    # Note: year_id isn't actually used by the decider. This is only included to allow
    # collapse.run.mics.special.age() to work correctly. 
    ready[, year_id := floor( (year_start + year_end) / 2) - cohort - 1]
