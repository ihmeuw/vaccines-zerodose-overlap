...
save_datestamp <- function() {
  ...
  date <- Sys.Date()
  date_extraction_root <- file.path("FILEPATH")
...
load_nid <- function(nid, folder) {
  ...
  # If missing, save error message and stop code
  if (length(file) < 1) {
    message("||---- File not found")
    cat("\nnidnotexist", file=file.path("FILEPATH"), append=TRUE)
...
# Make sure all needed columns exist, at least one vaccine column, and data contains at least 1 valid age
check_completeness <- function(data, cols, date, nid, team) {
  ...
  if (all(cols %in% names(data))) {
    if (length(vaccines[vaccines %in% names(data)]) > 0) {
      ...
      cat("\nnovaxind", file=file.path("FILEPATH"), append=TRUE)
      message("Missing at least one vaccine indicator; not moving on to tabulation")
      check <- FALSE
    }
  } else {
    cat("\nmissingind", file=file.path("FILEPATH"), append=TRUE)
    if (team=="lbd" & !"geo_id" %in% names(data) & !"geospatial_id" %in% names(data)) cat("\nmissinggeoind", file=file.path("FILEPATH"), append=TRUE)
  ...
  if ("age_year" %in% names(data)) { 
    if (nrow(data[!is.na(age_year)]) == 0) {
      cat("\nnoageind", file=file.path("FILEPATH"), append=TRUE)
...
make_rotac <- function(data) {
  ...
  doses <- readRDS("FILEPATH")[me_name=="vacc_rotac" & ihme_loc_id==unique(data$ihme_loc_id), .(doses)]
  intro <- readRDS("FILEPATH")[me_name=="vacc_rotac" & ihme_loc_id==unique(data$ihme_loc_id), cv_intro] %>% unique
...
#' Add age bins for age cohorting
#' This function is intended to determine the age cohorts for microdata (with specific ages) and lit data (with age ranges).
#' Three variables are introduced: `age_bin`, `age_bin_agg`, and `age_bin_agg_id`.
...
#'              age-range 0-11 months are given the age_bin_agg 1:3 to represent age bin 1 through 3, and `age_bin`` is set to NA.
#'              For children with specific ages (4 months, for example), `age_bin_agg`` is NA
...
#'                 is used. The age range is still signified by the `age_bin_agg` indicator, but `age_bin_agg_id` is set to TRUE to indicate
#'                 age-bin overlap.
add_age_bins <- function(data) {

  # Helper function to simplify assignment logic in data table
  ...
  # Get age bins and subset to relevant ranges
  age_bins <- fread("FILEPATH")
  age_bins <- age_bins[age_bin %in% c(1:11), ]

  # Determine whether data is microdata or lit data by checking column names. Microdata contains "age_year" (specific ages),
  # whereas lit data includes "age_start" and "age_end" (age ranges)
  is_microdata <- "age_year" %in% names(data)

  # If survey is for microdata, set age_start and age_end in order to match format of lit data and allow for merge with age_bins
  if (is_microdata) {
    
    # Format data types and check necessary indicators are present (should be redundant)
    ...
    # Get missing age_years from age_months if any (should be redundant)
    if (data[is.na(age_year) & !is.na(age_month), .N] > 0) data[is.na(age_year) & !is.na(age_month), age_year := floor(age_month / 12)]

    # Account for mismatch between age_year and age_months. Age_year is relied on in the case of disagreements
    data[age_year == 0 & age_month > 11, age_month := 11]
    
    # Set age_start and age_end indicators to merge with age_bins
  ...
  # Match ages to age bins where the specific age in months is known (microdata)
  ...
  # Match ages to age bins where the age range in months is known (lit data and microdata where age_year == 0 and age_month == NA, ie 0-11 months)
  data[age_start != age_end, age_bin := paste(age_bins[age_month_start >= age_start & age_month_end <= age_end, unique(age_bin)], collapse = ","), by = c("age_start", "age_end")]

  # For children that span multiple age bins (0-11 months), set "age_bin" to NA and "age_bin_agg" to <age_bin_min>:<age_bin_max>
  ...
  # For children that don't fit neatly in to age bins (should only occur in lit data, 18-29 months or 15-26 months for example),
  # assign to overlapping age bins and signify with TRUE in 'age_bin_agg_id'
  ...
  if (!is_microdata) {
    ...
    if (contains_mismatched_age_bins) {
      data[age_bin == "", `:=`(age_bin_agg_id = TRUE,
                               age_bin_agg = min_max(age_bins[(age_month_start <= age_start & age_month_end >= age_start & age_month_end <= age_end) |   # Age bin that overlaps bottom end of age range
                                                                (age_month_end >= age_end & age_month_start <= age_end & age_month_start >= age_start) | # Age bin that overlaps top end of age range
                                                                (age_month_start >= age_start & age_month_end <= age_end), unique(age_bin)])),           # Age bin that falls between ends of age range
           by = c("age_start", "age_end")]
    }
  }
  
  # Set missing age bins to NA
...
prep_for_tabulation <- function(nid, team="gbd", vaccines.=vaccines) {
  ...
  if (check) {
    ...
    # For LBD ordinal regression, drop children where we don't know exact number of doses (ie if DPT1/2/3 is 1/NA/NA, set to NA/NA/NA)
    if (team=="lbd") {
      for (vax_stem_col in unique(gsub("[0-9]", "", vaccines))) {
        if (paste0(vax_stem_col, "_dose") %in% names(data) & vax_stem_col %!in% c("mcv", "mmr", "rcv", "rota")) {
    ...
    if(team == "lbd"){
      ...
      # Convert column to correct data type, which defaults to logical when read in as empty
    ...
    # Assign year_id (used in outliering)
...
check_nids <- function(nid) {
  ...
  if (paste0(nid, ".csv") %in% list.files(file.path("FILEPATH")))     check_process <- "Success" else check_process <- "Failure"
  if (paste0(nid, ".csv") %in% list.files(file.path("FILEPATH"))) check_gbd     <- "Success" else check_gbd     <- "Failure"
  if (paste0(nid, ".csv") %in% list.files(file.path("FILEPATH"))) check_lbd     <- "Success" else check_lbd     <- "Failure"
  if (gbd & !lbd) { check_lbd <- "Not launched (GBD only)" }

  # read log messaging
  note <- readLines(file.path("FILEPATH"))
  ...
  write.csv(rbind(log, nid_log, fill=TRUE), file.path("FILEPATH"), row.names=FALSE)
...
clean_up <- function(nid, lbd=FALSE, gbd=FALSE) {
  ...
  if (paste0(nid, ".csv") %in% list.files(file.path("FILEPATH")))     system(paste0("rm ", file.path("FILEPATH")))
  if (paste0(nid, ".csv") %in% list.files(file.path("FILEPATH")))   system(paste0("rm ", file.path("FILEPATH")))
  if (gbd) if (paste0(nid, ".csv") %in% list.files(file.path("tabulated/gbd"))) system(paste0("rm ", file.path("FILEPATH")))
  if (lbd) if (paste0(nid, ".csv") %in% list.files(file.path("age_cohort/tabulated/lbd"))) system(paste0("rm ", file.path("FILEPATH")))
...
# List vaccines (order of doses matter - ascending!)
vax_info  <- fread("FILEPATH")
vaccines  <- vax_info$vaccine

# Indicators of interest for GBD full coverage models (used in GBD tabulate)
...
process <- function(nid) {
  ...
  # Save datestamp as confirmation of processing
  save_datestamp()
  ...
  # Identify whether survey being processed is MICS (processed differently)
  is_mics <- unique(data$survey_name) %in% c("UNICEF_MICS")
  
  # Remove measles doses from supplemental immunization campaigns
  drop_measles_sia <- TRUE
  
  # Calculate vaccination age in months for each vaccine
  calculate_age_at_vax <- FALSE
  
  # Set vaccine variables
  ...
  # Where birth dates are missing, determine birth dates from interview date and age_month
  ...
  # For MICS, drop antigens without both card and recall indicators (following MICS processing logic), unless 'response' variable is present
  if (is_mics){
    vacc_list <- c()
    ...
    # If any antigens were dropped due to indicator missingness, update list of vaccine stems
    ...
    # Message user if vaccines were dropped due to missingness
    if (length(vaccine_stems_dropped) > 0) {
      message(paste0("||---- Vaccines being dropped due to indicator missingness (MICS): ", paste(vaccine_stems_dropped, collapse = ", ")))
      vaccine_stems_in_dataset <- vaccine_stems_in_dataset[!vaccine_stems_in_dataset %in% vaccine_stems_dropped]
    }
  }
  
  # Stop code if no antigens are present
  ...
  # Assign values for "code" variables (i.e. numeric values that represent TRUE, FALSE, or missing for specific indicators) to variables in the environment
  ...
  # For "has_vacc_card' indicator, decode values (1 = seen, 2 = owns card but not seend, 3 = no card) and drop code columns
  ...
  #----DOSE-DETERMINATION-------------------------------------------------------------------------------------------------
  # For each indicator, determine dose based on card, recall, and response (if present)
  message("||-- Dose Determination")
  ...
  for (vacc in vaccines_all_in_dataset) {
    ...
    # Get vaccine stem, dose, and dose stem (ie for BCG, stem = "BCG", dose_num = 1, and dose_stem = "" since it's BCG not BCG1)
    ...
    if (!is_mics){
      ...
      #####################################
      ### 2. MATERNAL RECALL (NON-MICS) ###
      #####################################
      # Get doses from maternal recall
      ...
      # Set dose and dose from recall to number of recalled doses
      ...
      ###################################
      ### 3. CARD COVERAGE (NON-MICS) ###
      ###################################
      # Get doses from card 
      
      card_vacc_root <- paste0("card_", vacc_stem, dose_stem)
      
      # Prep card values for dose determination and split out dates for surveys Ubcov failed to split
      for (date_var in c("date")) {
        ...
        if (variable %in% names(data)) {
          ...
          # Convert card value to missing if value matches card_missing or card_recall codes 
          ...
          # Split dates into day/month/year variabes relying on card_date_format
      ...
      # Prep card values for dose determination
      for (date_var in c("day", "month", "year")) {
        # Set antigen-dose-specific variable
        variable <- paste0(card_vacc_root, "_", date_var)
        if (variable %in% names(data)) {
          # Convert dates to missing if date value is in card_missing, card_nodate, or card_recall value labels
          data[, (variable) := as.character(get(variable))]
          codes <- c("NA", ".", "")
          ...
          # Revert dates back to numbers (do not remove)
          data[, (variable) := as.integer(get(variable))]

          # Convert years from Buddhist calendar to Gregorian calendar
          if (unique(data$nid) %in% buddhist_nids & date_var=="year") data[, (variable) := get(variable) - 543]
          
          # Replace crazy date values with missingness. Retain day == 44 as code for confirmed no-date card dose
      ...
      # Get doses from cards with valid month/year indicators  
      if (paste0(card_vacc_root, "_month") %in% names(data) & paste0(card_vacc_root, "_year") %in% names(data)) {
        ...
        # If month and year vars not missing, replace doses with doses from month/year
      ...
      # Get doses from cards with "date" indicators - applies to cases we are unable to parse out day, month, and year
      if (paste0(card_vacc_root, "_date") %in% names(data)) {
        ...
        if (all(c(paste0(card_vacc_root, "_month"), paste0(card_vacc_root, "_year")) %in% names(data))) {
          ...
          # If month and year vars missing but date is not missing, replace doses with doses from date
          ...
          # If date is not missing, replace doses with doses from date
    ...
    if (is_mics){
      ...
      #################################
      ### 2. MATERNAL RECALL (MICS) ###
      #################################
      # Get doses from maternal recall
      
      card_vacc_root <- paste0("card_", vacc_stem, dose_stem)
      
      # Check whether recall indicator is present. MICS requires antigens to have both recall and card, except when response is present.
      # If antigen is present but recall is missing at this point in code, response is present.
      recall_is_present <- paste0("recall_", vacc_stem, "_ever") %in% names(data)
      
      # If recall ever is false, set the dose from recall to 0
      if (dose_num == 1) {
        # Check that recall is present (MICS antigen with no recall are dropped except when they have response variable)
      ...
      # For each dose, consider "66" in the day column (code for recall) or a valid number of recalled times as confirmation of dose received
      if (paste0(card_vacc_root, "_day") %in% names(data)) {
        if (dose_num == 1) {
          if(recall_is_present){
        ...
        # If day column is missing but date is present, interpret a valid number of recalled times as confirmation of dose received
        if (paste0(card_vacc_root, "_date") %in% names(data)) {
          if (dose_num == 1) {
            if(recall_is_present){
              # If recall_ever is 1, then we can say that there was a first dose recalled
              data[(get(paste0("recall_",vacc_stem, "_ever")) == 1),
                   c(paste0(vacc_stem, "_dose_from_recall")) := list((dose_num))]
            } 
          } else if (dose_num == 2) {
            if(recall_is_present){
              # If recall_ever is 1 AND recall is exactly 2, then we can say that the second dose was recalled	
              data[(get(paste0("recall_",vacc_stem, "_ever")) == 1 & paste0("recall_",vacc_stem, "_times") == dose_num),
                   c(paste0(vacc_stem, "_dose_from_recall")) := list((dose_num))]
            } 
          } else if (dose_num > 2) {
            if(recall_is_present){
              # If recall_ever is 1 AND recall is between dose_num and 7, give them the dose_num as recall	
      ...
      for (date_var in c("date")) {
        ...
        if (variable %in% names(data)) {
          ...
          # Convert card value to missing if value matches card_missing code 
          data[, (variable):= as.character(get(variable))]
          codes <- c("NA", ".", "")
          for (card_val in c("missing")) {
            # for (card_val in c("nodate", "recall")) {
            if (paste0("card_", card_val) %in% names(data)) codes <- c(codes, get(paste0("card_", card_val, "_code")))
          }
          data[get(variable) %in% unique(codes), (variable) := NA_character_]
          ...
          # Split dates into day/month/year variabes
          if ("card_date_format" %in% names(data)) {
            if (card_date_format_code %in% c("dd/mm/yyyy", "DD/MM/YYYY")) date_format_string <- "%d/%m/%Y"
            if (card_date_format_code %in% c("ddmmyy")) date_format_string <- "%d%m%Y"
          } else {
      ...
      # Prep card values for dose determination for surveys with card_<stem>_day variable (should be all MICS)
      for (date_var in c("day")) {
        ...
        if (variable %in% names(data)) {
          ...
          # Replace crazy date values with missingness. Retain day == 44 as code for confirmed no-date dose
          # Note this also removes the 66s which are dealt with in recall first! Warning if switching order
      ...
      if (paste0(card_vacc_root, "_date") %in% names(data)) {
        ...
        if (c(paste0(card_vacc_root, "_day")) %in% names(data)) {
          # If day variable is missing but date is not missing, replace doses with doses with date
    ...
    if (calculate_age_at_vax) {
      ...
      # Check for required age variables
    ...
    # Interpet doses from 'response' variable if present (dose determination made by survey series from card/recall - 
    # gold standard source of dose determination)
  ...
  # For MICS, set dose to the larger of card vs. recall
  if (is_mics){
  ...
  # Get component vaccines for each multi-vaccine
  if("pent" %in% vaccine_stems_in_dataset){
    ...
    if(unique(data$nid) %in% unconventional_penta_countries$survey_id){
      ...
      if(contains_hepb & !contains_hib){
        ...
        message(paste0('Dataset contains pentavalent vaccine comprised of unconventional component vaccines. Code is not currently designed
  ...
  # For each component vaccine of multi-vaccines
  for (multi_vacc in c("pent", "tetra", "mmr")) {
    
    if (multi_vacc %in% vaccine_stems_in_dataset) {
      
      # Message multi-vax in loop
      message(paste0("||---- ", multi_vacc))
      
      ###########################
      ### 1. DISTRIBUTE DOSES ###
      ###########################
      # Distribute doses to component vaccines
      
      for (col_in_loop in c("_dose", "_dose_from_card", "_dose_from_recall")) {
        # Set multi-vaccine vaccine object
      ...
      ### 2. SET CARD STATUS  ###
      ###########################
      # Set card status of component vaccines
      ...
      ### 3. AGE AT VAX       ###
      ###########################
      # Calculate age at vaccination for component vaccines
  ...
  # Fix max doses if too high, drop empty vaccine columns, and break out doses in to binary
  outvacc <- c()
  for (vacc_stem in all_stems) {
    if (paste0(vacc_stem, "_dose") %in% names(data)) {
      
      # Replace doses with max dose if too high
  ...
  # Message user
  message("||-- Dropping data with age missingness or outside age-range")
  
  # Age restrictions: drop data where age is missing or older than 5
  ...
  # Calculate dropped children from age missingness and age range *No data should be dropped prior to this point*
  n_total          <- nrow(data)
  ...
  # Save data drops to data drop table
  ...
  write.csv(data_drop_table, file = file.path("FILEPATH"), row.names = FALSE)
  ...
  message(paste0("||-- Process complete: ", nid))
  message("******************************************************\n")
