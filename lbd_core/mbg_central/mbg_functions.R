...
save_mbg_input <- function(indicator = indicator, indicator_group = indicator_group, df = df , simple_raster = simple_raster, simple_raster2 = NULL, mesh_s = mesh_s,
                           mesh_int = mesh_int, mesh_t = mesh_t, cov_list = all_cov_layers, run_date = NULL, pathaddin="",
  ...
  ## Save all inputs
  to_save <- c("df", "simple_raster", "mesh_s", "mesh_int", "mesh_t", 'cov_list', 'child_model_names', 'all_fixed_effects','period_map')
  if(!is.null(simple_raster2)) to_save <- c(to_save, "simple_raster2")
  save(list = to_save, file = paste0('<FILEPATH>/', indicator_group, '/', indicator, '/model_image_history/', run_date, pathaddin, '.RData'))
  message(paste0('All inputs saved to <FILEPATH>/', indicator_group, '/', indicator, '/model_image_history/', run_date, pathaddin,'.RData'))
...
#' @param use_adm2_res Logical. If TRUE, include subnational 
#'   (admin-1) random effects
#'   
...
build_mbg_data_stack <- function(df, 
  ...
  # else do the inla version
    ...
    if(use_ctry_res){
      ...
      design_matrix$CTRY.ID <- gaul_convert(df$country, shapefile_version=modeling_shapefile_version)
    }
...
fit_mbg <- function(indicator_family,
  ...
  inla_working_dir <- paste0("<FILEPATH>/inla_", run_date)
...
predict_mbg <- function(res_fit, cs_df, mesh_s, mesh_t, cov_list,
  ...
  run_dir <- paste0('<FILEPATH>/', indg, '/', ind, '/output/', rd, '/inla_draws')
  ...
  ## get uncertainty from adm2 res
  # if (!is.null(pred_adm2_res)) {
  #   cell_adm2_res_sd <- apply(cell_adm2_res, 1, sd)
  # }
  ...
  #  # add adm2 res if present
  # if (!is.null(pred_adm2_res)) {
  #   cell_sd <- cell_sd + cell_adm2_res_sd^2
  # }
...
save_mbg_preds <- function(config, time_stamp, run_date, mean_ras, sd_ras, res_fit, cell_pred, df ,pathaddin="") {
  ...
  if(time_stamp==TRUE) output_dir <- paste0('<FILEPATH>/', indicator_group, '/', indicator, '/output/', run_date)
  if(time_stamp==FALSE) output_dir <- paste0('<FILEPATH>/', indicator_group, '/', indicator, '/output/scratch')
